  ...
  >
    <div class="aspect-video bg-gray-700 relative">
      ...
      <div class="absolute top-2 left-2 flex gap-2">
        ...
        <span
          v-if="map"
          class="bg-gray-900/80 text-white text-xs px-2 py-1 rounded"
        >
          {{ map.displayName }}
        </span>
      </div>
      <div
        v-if="lineup.side || lineup.site"
        class="absolute bottom-2 right-2 flex gap-1.5"
      >
        <span
          v-if="lineup.side"
    ...
    <div class="p-4">
      ...
      <div class="flex items-center justify-between text-sm text-gray-400">
        <span
          ...
          @click.prevent.stop="
            navigateTo(`/profile/${lineup.profile.username}`)
          "
        >
          <svg
