<template>
  <div>
    ...
    <div
      v-if="
        profile && (activeTab === 'collections' || activeTab === 'subscribed')
      "
    >
      ...
      <div v-else-if="activeTab === 'subscribed'">
        <div class="flex items-center justify-between mb-4">
          <h2 class="text-lg font-semibold text-white">
            ...
            <span
              v-if="subscribedCollections.length"
        ...
        <!-- Infinite scroll sentinel -->
        <div
          ref="subscribedCollectionsSentinel"
          class="h-10 flex items-center justify-center mt-4"
        >
          <span
            v-if="subscribedCollectionsLoading && subscribedCollections.length"
    ...
    <div v-else-if="profile && activeTab === 'bookmarked'">
      ...
      <div
        v-if="bookmarkFoldersLoading && !bookmarkFolders.length"
        class="text-gray-400"
      >
        Loading...
      </div>
  ...
  <Teleport to="body">
    ...
    >
      ...
      />
      <div
        class="relative bg-gray-800 rounded-lg p-6 max-w-md w-full mx-4 shadow-xl"
      >
...
const { data: lineupsCount } = await useAsyncData(
  `profile-lineups-count-${username}`,
  async () => {
...
// Tab switching
function switchTab(
  tab: "lineups" | "liked" | "bookmarked" | "collections" | "subscribed",
) {
...
async function fetchLineups(reset = false) {
  ...
  if (activeTab.value === "liked") {
    ...
    if (filters.value.map)
      query = query.eq("lineups.map_uuid", filters.value.map);
    if (filters.value.agent)
      query = query.eq("lineups.agent_uuid", filters.value.agent);
    if (filters.value.abilities.length)
      query = query.in("lineups.ability", filters.value.abilities);
    if (filters.value.site)
      query = query.eq("lineups.site", filters.value.site);
    if (filters.value.side)
      query = query.eq("lineups.side", filters.value.side);
    ...
    if (filters.value.agent)
      query = query.eq("agent_uuid", filters.value.agent);
    if (filters.value.abilities.length)
      query = query.in("ability", filters.value.abilities);
...
async function fetchSubscribedCollections(reset = false) {
  ...
  const items = (data ?? [])
    .map(
      (row: {
        collections: RawCollectionWithCounts & Record<string, unknown>;
      }) => row.collections,
    )
    .filter(Boolean)
...
async function fetchBookmarkFolders(reset = false) {
  ...
  const items = (data ?? []).map(
    (item: RawBookmarkFolderWithCount & Record<string, unknown>) => ({
      ...item,
      lineups_count: item.lineups_count?.[0]?.count ?? 0,
...
onMounted(() => {
  ...
  if (bookmarkFoldersSentinel.value)
    bookmarkFoldersObserver.observe(bookmarkFoldersSentinel.value);
});
